...
const tabFrame = (tab) => {
  return iframe({
    ...
    onload: (e) => {
      // Skip onload processing for local pages
...
function focusTab(tab) {
  ...
  // Update URL bar
  urlInput.value = tab.isLocal ? "" : tab.url;
...
async function addTab(link) {
  ...
  tab.isLocal = link.startsWith("/");
  tab.title = tab.isLocal ? "New Tab" : decodeURIComponent(
    __uv$config.decodeUrl(url.substring(url.lastIndexOf("/") + 1))
  ).replace(/^https?:\/\//, "");
  tab.url = tab.isLocal ? link : search(link);
  tab.proxiedUrl = url;
  tab.icon = null;
